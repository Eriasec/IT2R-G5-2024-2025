...
/******************************************************************************/
// Dernière modification le 27/11/2015 - XM 
...
/*******************************************************************************
* Draw bargraphHvB   (en partant du haut)                                                              *
*   Parameter:      x:        horizontal position                              *
*                   y:        vertical position                                *
*                   w:        maximum width of bargraph (in pixels)            *
*                   h:        bargraph height                                  *
...
void GLCD_BargraphHvB (unsigned int x, unsigned int w, unsigned int h) {
  int i,j;

  GLCD_SetWindow(x, 20, w, h);
  wr_cmd(0x22);
  wr_dat_start();
  for (i = 0; i < h; i++) {
    for (j = 0; j <= w-1; j++) {
...
/*******************************************************************************
* Draw bargraphBvH   (en partant du bas)                                                             *
*   Parameter:      x:        horizontal position                              *
*                   w:        maximum width of bargraph (in pixels)            *
*                   h:        bargraph height                                  *
*                              *
...
void GLCD_BargraphBvH (unsigned int x, unsigned int w, unsigned int h) {
  int i,j;
	if (h<1) h=1;
	if (h>220) h=220;
	GLCD_SetWindow(x, 240-h, w, h);
  wr_cmd(0x22);
  wr_dat_start();
  for (i = 0; i < h; i++) {
    for (j = 0; j <= w-1; j++) {
...
/*******************************************************************************
*Fonction de dessin sur l'écran LCD - IUT de Cachan           *
*   Parameter:      reg:      lcd register address                             *
*                   val:      value to write into reg                          *
*   Return:                                                                    *
*******************************************************************************/
...
{
	GLCD_SetTextColor(couleur);
	GLCD_PutPixel(x0,y0);
}


/*
Algorithme sur wikipedia:
...
void Ligne_horiz(int x0, int x1, int y)
{
	...
	for(x=x0;x<(x1+1);x++) GLCD_PutPixel(x,y);
}
...
void Ligne_verti(int x, int y0, int y1)
{
	...
	for(y=y0;y<(y1+1);y++) GLCD_PutPixel(x,y);
}
...
void Cercle_plein(int x0,int y0, int rayon)
{
	...
	while(x >= y)
	{
		Ligne_horiz(-x + x0, x + x0, y + y0);
		Ligne_horiz(-x + x0, x + x0, -y + y0);
		Ligne_horiz(-y + x0, y + x0, x + y0);
		Ligne_horiz(-y + x0, y + x0, -x + y0);
...
{
	GLCD_SetTextColor(couleur);
	Cercle_plein(x0, y0, rayon);
}

void Ellipse_vide(int x0, int y0, int rayon_x, int rayon_y)
{
	int a2=(rayon_x*rayon_x);
	int b2=(rayon_y*rayon_y);
	
	int x=rayon_x;
	int y=0;
	
	int x_change=b2*(1-(rayon_x<<1));
	int y_change=a2;
	
	int error=0;
	int stop_x=(b2<<1)*rayon_x;
	int stop_y=0;
...
{
	GLCD_SetTextColor(couleur);
	Ellipse_vide(x0, y0, rayon_x, rayon_y);
}
...
{
	GLCD_SetTextColor(couleur);
	Ellipse_plein(x0, y0, rayon_x, rayon_y);
}

void Rectangle_vide(int x0, int y0, int cote_x, int cote_y)
{
	Ligne_horiz(x0,x0+cote_x,y0);
	Ligne_verti(x0+cote_x,y0,y0+cote_y);
	Ligne_horiz(x0,x0+cote_x,y0+cote_y);
	Ligne_verti(x0,y0,y0+cote_y);
}

void Rectangle_vide_c(int x0, int y0, int cote_x, int cote_y, short couleur)
{
	GLCD_SetTextColor(couleur);
	Rectangle_vide(x0,y0,cote_x,cote_y);
}

void Rectangle_plein(int x0, int y0, int cote_x, int cote_y)
{
	int y;
	for (y=y0;y<(y0+cote_y+1);y++) Ligne_horiz(x0,x0+cote_x,y);
}

void Rectangle_plein_c(int x0, int y0, int cote_x, int cote_y, short couleur)
{
	GLCD_SetTextColor(couleur);
	Rectangle_plein(x0,y0,cote_x,cote_y);
}

/*******************************************************************************
* Affichage d'une valeur sur l'écran LCD (Code Poste) cf TP2 IENA S3           *
*   Parameter:      reg:      lcd register address                             *
*                   val:      value to write into reg                          *
*   Return:                                                                    *
*******************************************************************************/
...
// Affichage d'une valeur sur l'écran LCD (Code acheminement)
// ----------------------------------------------------------
void AfficheLCD_code(short sortie)
{
	switch (sortie)
	{
		case 0x01:  GLCD_DisplayChar (5, 5, 1, '0');
								break;
		case 0x02:  GLCD_DisplayChar (5, 5, 1, '1');
								break;
		case 0x04:  GLCD_DisplayChar (5, 5, 1, '2');
								break;
		case 0x08:  GLCD_DisplayChar (5, 5, 1, '3');
								break;
		case 0x10:  GLCD_DisplayChar (5,5, 1, '4');
								break;
		case 0x20:  GLCD_DisplayChar (5, 5, 1, '5');
								break;
		case 0x40:  GLCD_DisplayChar (5, 5, 1, '6');
								break;
		case 0x80:  GLCD_DisplayChar (5, 5, 1, '7');
								break;
		case 0x100: GLCD_DisplayChar (5, 5, 1, '8');
								break;
		case 0x200: GLCD_DisplayChar (5, 5, 1, '9');
								break;
		default:		GLCD_DisplayChar (5, 5, 1, 'X');
								break;
	}
